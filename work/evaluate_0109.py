...
def evaluate_0109():
    ...
    results_path = 'data/results_20260109.json'
    ...
    for rid in races:
        ...
        try:
            ...
            for bet in recommendations['recommendations']:
                ...
                actual_payouts_dict = actual_results.get('payouts', {}).get(bet_type, {})
                if actual_payouts_dict:
                    combinations = actual_payouts_dict.get('combinations', [])
                    payout_amounts = actual_payouts_dict.get('payouts', [])
    ...
    # Generate Report
    report_lines = []
    report_lines.append("# 2026/01/09 予測パフォーマンス評価レポート")
    ...
    for r in race_results:
        report_lines.append(f"| {r['track']} {int(r['num'])}R | {r['invested']}円 | {r['payout']}円 | {int(r['profit'])}円 | {r['hits']}/{r['bets']} |")
    
    report_path = Path('reports/evaluation_20260109.md')
